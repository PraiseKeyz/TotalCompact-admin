...
const UploadProject = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.zipCode) formDataToSend.append('location[zipCode]', formData.zipCode);
    formDataToSend.append('status', formData.status);
    formDataToSend.append('projectType', formData.projectType);
    if (formData.estimatedCompletionDate)
      formDataToSend.append('estimatedCompletionDate', formData.estimatedCompletionDate);
    if (formData.totalUnits) formDataToSend.append('totalUnits', formData.totalUnits);
    if (formData.priceMin) formDataToSend.append('priceRange[min]', formData.priceMin);
    if (formData.priceMax) formDataToSend.append('priceRange[max]', formData.priceMax);
    formData.features.forEach((feature) => {
      if (feature) formDataToSend.append('features[]', feature);
    });
    formData.images.forEach((image) => {
      if (image.file) formDataToSend.append('images', image.file);
      if (image.caption) formDataToSend.append('imageCaptions[]', image.caption);
    });

    try {
      const res = await axios.post(`${API_BASE_URL}/api/v1/projects/create`, formDataToSend, {
        headers: {
      ...
      setSubmitMessage(`Error: ${(err).response?.data?.error || 'Something went wrong'}`);
    } finally {
      setIsSubmitting(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600">Estimated Completion Date</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600">Total Units</label>
              <input
                type="number"
                name="totalUnits"
        ...
        <div className="space-y-4">
          ...
          {formData.images.map((img, i) => (
            <div key={i} className="space-y-2 border p-4 rounded bg-gray-50">
              <div>
                <label className="block text-sm font-medium text-gray-600">Image File</label>
                <input
                  type="file"
                  name="images"
                ...
                />
                {img.file && (
                  <p className="text-sm text-gray-500 mt-1">Selected: {img.file.name}</p>
                )}
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-600">Image Caption</label>
                <input
                  type="text"
                  name="images"
                  value={img.caption}
